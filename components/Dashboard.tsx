...
const Dashboard: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="w-full min-h-screen bg-[#080808] text-white py-24 px-4 md:px-12 relative z-20">
      <div className="max-w-7xl mx-auto">
        
        {/* Header */}
        <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between border-b border-gray-800 pb-8">
          <div>
            <h2 className="text-4xl md:text-6xl font-bold tracking-tighter mb-2">实时数据</h2>
            <p className="font-mono text-gray-500">狼姐 // 直播数据分析</p>
          </div>
          ...
          {MOCK_METRICS.map((metric, idx) => (
            ...
            >
              ...
              <div className="font-mono text-xs text-gray-500 mb-4 flex justify-between items-center">
                ...
                <span className={`px-2 py-0.5 text-[10px] ${metric.status === 'LIVE' ? 'bg-green-900/30 text-green-400 border border-green-900' : 'bg-red-900/30 text-red-400 border border-red-900'}`}>
                  {metric.status}
                </span>
              </div>
              ...
              <div className="font-mono text-sm text-gray-400 mb-4">
                {metric.viewers.toLocaleString()} <span className="text-[10px]">观众</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 chart-section">
          ...
          <div className="lg:col-span-2 bg-[#0F0F0F] border border-gray-800 p-6 chart-container relative group hover:border-gray-700 transition-colors">
             ...
             <div className="flex justify-between items-center mb-8">
               ...
               <div className="flex gap-2 font-mono text-xs">
                 <span className="px-2 py-1 bg-yellow-500 text-black font-bold cursor-pointer">实时</span>
                 <span className="px-2 py-1 bg-gray-800 text-gray-400 cursor-pointer hover:bg-gray-700 hover:text-white transition-colors">历史</span>
               </div>
             </div>
             ...
             <div className="h-[300px] w-full">
               <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={CHART_DATA}>
                   ...
                   <XAxis dataKey="time" stroke="#444" tick={{fontSize: 12, fontFamily: 'monospace'}} />
                   <YAxis stroke="#444" tick={{fontSize: 12, fontFamily: 'monospace'}} />
                   <Tooltip content={<CustomTooltip />} />
                   <Area type="monotone" dataKey="value" stroke="#EAB308" fillOpacity={1} fill="url(#colorVal)" strokeWidth={2} />
                   <Area type="monotone" dataKey="value2" stroke="#666" fillOpacity={1} fill="url(#colorVal2)" strokeWidth={1} strokeDasharray="5 5" />
                 </AreaChart>
               </ResponsiveContainer>
             </div>
          </div>
          ...
          <div className="bg-[#0F0F0F] border border-gray-800 p-6 chart-container flex flex-col relative group hover:border-gray-700 transition-colors">
            ...
            <div className="mt-6 pt-4 border-t border-gray-800">
               <a href="#" className="flex items-center justify-between text-xs font-mono text-gray-500 hover:text-white transition-colors group/link">
                  <span>查看完整报告</span>
